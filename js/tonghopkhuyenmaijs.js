...
const imgNumber = document.querySelectorAll('.slide-khuyenmai_item');
let current=0;
rightbtn.addEventListener("click",function(){
    current++
    if(current>imgNumber.length-3){
        current=0
    }
    document.querySelector(".slide-khuyenmai_center").style.right=current*50+"%"
})
leftbtn.addEventListener("click",function(){
    current--
    if(current<=0){
        current=imgNumber.length-3
    }
    document.querySelector(".slide-khuyenmai_center").style.right=current*50+"%"
})
// locate
